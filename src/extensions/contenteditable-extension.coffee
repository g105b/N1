...
class ContenteditableExtension
  ...
  Must return an array of objects obeying the following spec:
    ...
    will get passed an single object with the following args.
  ...
  ###
  Public: Override `toolbarComponentData` to declaratively show your own
  ...
  Must return an object with the following signature
    ...
    - locationRef: Anything (usually a DOM Node) that responds to
